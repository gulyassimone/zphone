if Config.Core == "QBX" then
  ...
  xCore.GetPlayersByJob = function(jobName)
    ...
    local players = {}
    local qbPlayers = QBX.Functions.GetQBPlayers()

    for _, ply in pairs(qbPlayers or {}) do
      if ply.PlayerData and ply.PlayerData.job and ply.PlayerData.job.name == jobName then
        local wrapped = xCore.GetPlayerBySource(ply.PlayerData.source)
        if wrapped then
