...
import {
  CLOSE_CALL,
  BOTTOM_MENUS as DEFAULT_BOTTOM_MENUS,
  MENUS as DEFAULT_MENUS,
  MENU_ADS,
  MENU_BANK,
  MENU_CAMERA,
  MENU_CONTACT,
...
const parseCreatedAtValue = (value) => {
...
const sortNewsByCreatedAt = (list) => {
  if (!Array.isArray(list)) return [];
  return [...list].sort((a, b) => parseCreatedAtValue(b?.created_at) - parseCreatedAtValue(a?.created_at));
};
...
function App() {
  ...
  const fromLabel = t("from_label");
  const outsideMessageTitle = t("new_message_title");
  const outsideCallTitle = t("incoming_call_title");
  const prevServiceReportsLenRef = useRef(
    Array.isArray(services?.reports) ? services.reports.length : 0
  );
  ...
  function generateDimensions(height) {
    ...
    const aspectRatio = initWidthAndHeight.initHeight / initWidthAndHeight.initWidth;
    const newWidth = height / aspectRatio;
    const newRadius = height * 0.066;
    const newMargin = height * 0.033;
    const newScale = (height / initWidthAndHeight.initHeight) * resolution.scale;
  ...
  useEffect(() => {
    const currentLen = Array.isArray(services?.reports)
      ? services.reports.length
      : 0;
    if (currentLen > prevServiceReportsLenRef.current) {
      setNotificationMessage({
        type: MENU_NEW_MESSAGE_NOTIFICATION,
        from: t("menu_services"),
        is_service: true,
      });
    }
    prevServiceReportsLenRef.current = currentLen;
  }, [services?.reports]);
  ...
  const getNews = async () => {
    console.log("[z-phone] getNews -> POST /get-news");
    let result = [];
    try {
      const response = await axios.post("/get-news");
      result = response.data;
      console.log(
        "[z-phone] /get-news response",
        JSON.stringify({
          newsCount: Array.isArray(result?.news) ? result.news.length : null,
          streamsCount: Array.isArray(result?.streams)
            ? result.streams.length
            : null,
          sample: Array.isArray(result?.news) && result.news[0]
            ? {
                id: result.news[0].id,
                title: result.news[0].title,
                source: result.news[0].source,
                created_at: result.news[0].created_at,
              }
            : null,
        })
      );
    } catch (error) {
      console.error("[z-phone] error /get-news", error);
    }

    setNews(sortNewsByCreatedAt(result.news));
  ...
  const handleEventPhone = (event) => {
    ...
    console.log("[z-phone] handleEventPhone", JSON.stringify(data));

    // News handled via single get-news axios call; ignore NUI pushes
    ...
    if (data.closeCall) {
      ...
      if (data.notification) {
        ...
        setNews(sortNewsByCreatedAt(data.news));
    ...
    fetch("static/config.json") // Adjust the path accordingly
      ...
      .then((data) => {
        setMenus({
          ...data,
          MENUS: DEFAULT_MENUS,
          BOTTOM_MENUS: DEFAULT_BOTTOM_MENUS,
        });
