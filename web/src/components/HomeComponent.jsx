...
const HomeComponent = ({ isShow }) => {
  ...
  return (
    ...
    >
      <div
        className="py-2 px-6 text-green-800 grid grid-cols-3 gap-3 justify-items-center"
        style={{
          paddingTop: 50,
        }}
      >
        {menus.MENUS.map((v, i) => {
          return (
            <div
              onClick={() => setMenu(v.menu)}
              key={i}
              className="relative w-14 h-14 flex flex-col items-center mb-6 cursor-pointer"
            >
              ...
              <img className="rounded-xl w-14 h-14" src={v.icon} alt="" />
              <div>
                <p className="text-sm text-white font-normal text-center leading-tight">
                  {v.label}
                </p>
              </div>
            </div>
