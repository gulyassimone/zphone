...
export const MENUS = (config.MENUS || [])
  .map((item) => ({
    ...item,
    // Keep original identifier for routing, show translated text on UI
    menu: item.menu || item.label,
    label: translateLabel(item.label),
  }));

export const BOTTOM_MENUS = (config.BOTTOM_MENUS || []).map((item) => ({
  ...item,
  menu: item.menu || item.label,
  label: translateLabel(item.label),
}));
