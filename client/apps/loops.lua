...
RegisterNUICallback('send-tweet', function(body, cb)
  ...
  if not Net.ensureData(Config.App.InetMax.InetMaxUsage.LoopsPostTweet) then
    cb(false)
    return
  end
  ...
  lib.callback('z-phone:server:SendTweet', false, function(isOk)
    if isOk then
      Net.consume(Config.App.Loops.Name, Config.App.InetMax.InetMaxUsage.LoopsPostTweet)
    end
    lib.callback('z-phone:server:GetTweets', false, function(tweets)
      cb(tweets)
    end)
...
RegisterNUICallback('send-tweet-comments', function(body, cb)
  ...
  if not Net.ensureData(Config.App.InetMax.InetMaxUsage.LoopsPostComment) then
    cb(false)
    return
  end
  ...
  lib.callback('z-phone:server:SendTweetComment', false, function(isOk)
    if isOk then
      Net.consume(Config.App.Loops.Name, Config.App.InetMax.InetMaxUsage.LoopsPostComment)
    end
    cb(isOk)
  end, body)
end)
