local logger = exports and exports['simi_logger'] and exports['simi_logger']:getSharedObject()

if not logger then
  error('[z-phone] simi_logger not loaded; start simi_logger before z-phone')
end

Log = logger
--Log.OVERRIDE_LEVEL = 'debug'
Log.CALLER_ENABLED = true
--Log.setLevel('debug')

Config = {}

-- Language used for server-side messages (matches keys in web/src/locale.json)
Config.Locale = "hu"
...
Config.App = {
  InetMax = {
    Name = "InetMax",
    EnableDataUsage = false, -- if false, skip internet data tracking/charges
    IsUseInetMax = true,
    TopupRate = {
      InKB = 1000000, -- 1 GB
      Price = 100
    ...
    InetMaxUsage = {
      ...
      BankCheckTransferReceiver = math.random(50, 150000),
      BankTransfer = math.random(10, 20000000),
    },
